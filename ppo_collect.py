...
def get_env(env_str, lower_bound, upper_bound):
...
def get_model(env, env_str, algorithm_str):
    ...
    model_path = f'../alt_drones/results/dense/{env_name}/{algorithm_str}.zip'
    #model_path = f'../alt_drones/backup/{algorithm_str}{env_name}.zip'
    #model_path = f'../alt_drones/ppo_only/backup/dense/06_basic_z_10/{env_name}/{algorithm_str}.zip'
...
def main(env_num=DEFAULT_ENV, algo_num=DEFAULT_ALGO,
    ...
    for _ in tqdm(range(multiplier*NUM_SAMPLES)):
        if(algo_num == 0):
...
if __name__ == '__main__':
    ...
    parser.add_argument('multiplier', type=int, default=DEFAULT_SAMPLE_MULT,
            help='multiplier for number of samples')
